...
注册成功后 Worker 每 10 秒向 `/api/v1/workers/{worker_id}/heartbeat` 上报。空闲 Worker 上报 `available_batch_slots=1`、`configured_pool_workers` 与 `max_seeds_per_batch`；Master 在此心跳响应中签发任务或返回空分配。Master 不再向 Worker 主动 POST 任务，也不由 Worker 自己挑选 Seed。
