...
工作台布局必须支持用户临时调整：左列与中列的右边界均可横向拖拽，右列自动占据其余可用宽度；左栏可在算法、问题、执行设置之间纵向拖拽，Worker 区域占用余下空间。每个区域需保留最小尺寸，目录列表、Worker 列表和执行设置在其各自区域内部滚动。布局尺寸仅保存在当前浏览器会话；中栏顶部只保留“导入”和“保存”配置操作，不显示独立的“实验预设”选择行。
